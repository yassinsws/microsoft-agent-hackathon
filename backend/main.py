...
MOCK_PROPERTY_TEMPLATES = [
    {
        ...
        "location": {"city": "Munich", "neighborhood": "Bogenhausen", "state": "Bavaria"},
    ...
    },
    {
        ...
        "location": {"city": "Munich", "neighborhood": "Maxvorstadt", "state": "Bavaria"},
    ...
    },
    {
        ...
        "features": ["Garden", "Garage", "Traditional Architecture", "Updated Kitchen", "Basement"]
    }
]
...
@app.post("/api/property/upload", response_model=PropertyUploadResponse, status_code=201)
...
@app.post("/api/property/generate/{property_id}", response_model=PropertyGenerationResponse)
def generate_property_listing(property_id: str, generation_request: Optional[PropertyGenerationRequest] = None):
    ...
    if upload_data["processed"]:
        raise HTTPException(status_code=400, detail="Property listing already generated")
    ...
    def generate_pricing_analysis(proposed_price: int, property_details: dict, location: dict) -> PricingAnalysis:
    ...
    ai_property = AIGeneratedProperty(
        ...
        pricePerSqft=round(selected_template["price_range"][1] / selected_template["details"]["sqft"]),
        location=PropertyLocation(
...
def create_item_legacy(input: dict):
    """Legacy endpoint for backward compatibility."""
    return {"price": 10, "description": "Legacy endpoint - use /api/property/ endpoints instead"}
